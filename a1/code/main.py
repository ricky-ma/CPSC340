...
if __name__ == "__main__":
    ...
    if question == "3.4":
        ...
        mean = X.mean()
        median = np.median(X)
        mode = stats.mode(X, axis=None)
        print(min, max, mean, median, mode)

        five = np.percentile(X, 5)
        twofive = np.percentile(X, 25)
        fifty = np.percentile(X, 50)
        sevenfive = np.percentile(X, 75)
        ninefive = np.percentile(X, 95)
        print(five, twofive, fifty, sevenfive, ninefive)

        colmeans = np.mean(X, axis=0)
        colvars = np.var(X, axis=0)
        print("min-mean:" + names[colmeans.argmin()])
        print("max-mean:" + names[colmeans.argmax()])
        print("min-var:" + names[colvars.argmin()])
        print("max-var:" + names[colvars.argmax()])
