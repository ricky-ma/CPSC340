    ...
    def fit(self, X, y):
        ...
        for d in range(D):
            for n in range(N):
                ...
                count_yes = np.bincount(y[X[:, d] > value])
                count_no = np.bincount(y[X[:, d] <= value])
                ...
                # infogain = entropy(y_new) - (count_sat/N)*entropy(y_sat) - (count_not/N)*entropy(y_not)
                infogain = entropy(y/np.sum(y)) - (n_yes/N)*entropy(count_yes/n_yes) - (n_no/N)*entropy(count_no/n_no)
